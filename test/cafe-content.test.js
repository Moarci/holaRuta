/*
 * cafe-content.test.js – Schützt Vollständigkeit & Verdrahtung des Café-Moduls
 * „Café de la región" (cafe.js): Kaffeeanbau & -kultur. Fängt dieselben
 * Fehlerklassen wie flirt-content.test.js / flyer-modules-content.test.js:
 *
 *  - Schema: INTRO(+EN), TOPICS, PHRASES, GLOSSARY, CHECKLIST – sonst rendert
 *    ui.renderCafe (moduleSheet) eine leere Seite.
...
require(path.join(SRC, "cafe.js"));
const { cafe, i18n } = globalThis.window.SC;

test("cafe: lädt und exportiert das Info-Modul-Schema", () => {
  assert.ok(cafe, "window.SC.cafe fehlt (vor app.js geladen?)");
  assert.ok(typeof cafe.INTRO === "string" && cafe.INTRO.trim().length, "INTRO fehlt");
  assert.ok(typeof cafe.INTRO_EN === "string" && cafe.INTRO_EN.trim().length, "INTRO_EN fehlt");
  for (const k of ["TOPICS", "PHRASES", "GLOSSARY", "CHECKLIST"]) {
    assert.ok(Array.isArray(cafe[k]) && cafe[k].length, `${k} fehlt oder leer`);
  }
});

test("cafe.TOPICS: jedes Thema ist zweisprachig & ausgewogen", () => {
  cafe.TOPICS.forEach((tp, i) => {
...
test("cafe.TOPICS: Lesetraining ist vollständig & kohärent (es/vocab/level)", () => {
  ...
  assert.ok(withReading >= 3, `cafe: zu wenige Themen mit Lesetraining (${withReading})`);
});

test("cafe.PHRASES: jede Satzgruppe & jeder Satz ist vollständig (es/de/en)", () => {
  const ids = new Set();
  cafe.PHRASES.forEach((g, i) => {
...
test("cafe.GLOSSARY: vollständige es/de/en-Felder", () => {
  cafe.GLOSSARY.forEach((w, i) => {
    assert.ok(w.es && w.de && w.en, `GLOSSARY ${i}: es/de/en unvollständig`);
  });
});

test("cafe.CHECKLIST: vollständige DE/EN-Felder", () => {
  cafe.CHECKLIST.forEach((c, i) => {
...
test("cafe: i18n-Schlüssel (Kachel + Überschriften) existieren in DE & EN", () => {
  const prev = i18n.getLang();
  const keys = [
    "discover.subCafe",
    "discover.cfTips", "discover.cfPhrases", "discover.cfWords",
    "discover.cfChecklist", "discover.cfChecklistHint",
  ];
  try {
    ["de", "en"].forEach((lang) => {
      i18n.setLang(lang);
