...
(function () {
  ...
  // Felder, die je Gerät kumulieren: über Geräte hinweg MAX (statt Summe),
  // damit sich nichts durch überlappende Historie aufbläht.
  var MAX_FIELDS = {
    reviews: 1, againPresses: 1, dailyStreak: 1, longestStreak: 1,
    battlesPlayed: 1, battlesWon: 1, perfectBattles: 1, comebacks: 1,
    quizzesPlayed: 1, quizzesPerfect: 1, frasesPlayed: 1, frasesPerfect: 1,
    listenReviews: 1, preciosPlayed: 1, preciosPerfect: 1, preciosMillon: 1,
    conjugPlayed: 1, conjugPerfect: 1, dialogosPlayed: 1, dialogosPerfect: 1,
  };
  ...
  function mergeGamestats(a, b) {
    ...
    for (k in keys) {
      ...
      if (k === "tripGoal") {
        ...
        out[k] = MAX_FIELDS[k] ? Math.max(va, vb) : Math.max(va, vb);
  ...
  // Eigene Karten: Vereinigung per id (lokale Variante gewinnt bei Kollision).
  function mergeUsercards(a, b) {
    var la = Array.isArray(a) ? a : [], lb = Array.isArray(b) ? b : [];
    var byId = {}, order = [], i, c;
    for (i = 0; i < lb.length; i++) { c = lb[i]; if (c && c.id) { if (!(c.id in byId)) order.push(c.id); byId[c.id] = c; } }
    for (i = 0; i < la.length; i++) { c = la[i]; if (c && c.id) { if (!(c.id in byId)) order.push(c.id); byId[c.id] = c; } }
